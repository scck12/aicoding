(function () {
  if (window.__partySlotSearchBound) return;
  window.__partySlotSearchBound = true;
  ...
  function renderResults(container, partyId, slot, items) {
    ...
    for (var i = 0; i < items.length; i++) {
      var pokemon = items[i];
      var title = pokemon.nameKo || pokemon.nameEn || "#" + pokemon.id;
      var en = pokemon.nameEn ? " \u00b7 " + escapeHtml(pokemon.nameEn) : "";
      var idLabel = "#" + String(pokemon.id).padStart(4, "0");
      html +=
        "<li>" +
        ...
        '<input type="hidden" name="pokemonId" value="' +
        pokemon.id +
        '" />' +
        ...
        '<span style="display:flex;height:44px;width:44px;flex-shrink:0;align-items:center;justify-content:center;border-radius:8px;background:#fafafc;">' +
        '<img src="' +
        escapeHtml(pokemon.image) +
        '" alt="' +
        escapeHtml(title) +
        '" width="40" height="40" style="height:40px;width:40px;object-fit:contain;" />' +
        "</span>" +
  ...
  document.addEventListener(
    ...
    function (event) {
      ...
      if (target.getAttribute("data-party-search") !== "1") return;

      var slot = target.getAttribute("data-slot");
      var partyId = target.getAttribute("data-party-id");
      var container = document.getElementById("party-search-results-" + slot);
      if (!slot || !partyId || !container) return;

      window.clearTimeout(window.__partySlotSearchTimer);
      window.__partySlotSearchTimer = window.setTimeout(function () {
        ...
        fetch(
          "/api/pokemon/search?q=" + encodeURIComponent(query) + "&limit=12",
        )
